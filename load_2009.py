...
lista_indicadores = ['28242']
cols = ['id_MUNICIPIO', 'Estabelecimentos_SUS']
#monta string de municipios
df_mun = df_geral['id_MUNICIPIO']

inicio_count=0
fim_count=10
final_2009 = pd.DataFrame()
while (fim_count <= 5570):
	try:
		...
		print(len(populacao_residente_df))
		
		try:
			populacao_residente_df.res = populacao_residente_df.res.apply(lambda x: str(x[0]['res']['2009']))
			#print('a')
		...
		final_2009 = pd.concat([final_2009, populacao_residente_df])
		#time.sleep(2)
	except Exception as e:
		print(str(e))
		#print(populacao_residente_df.res[0])
		final_2009.to_excel("final_bkp.xlsx")
	#print(final_2009)


final_2009.columns = cols
final_2009.set_index('id_MUNICIPIO', inplace=True)
final_2009.to_excel("final_2009.xlsx")
