...
lista_indicadores = ['47001','29749', '28141', '78187', '78192', '30279']
cols = ['id_MUNICIPIO',  'Total_Receitas',  'Despesas_Empenhadas', 'Mortalidade_Infantil', 'Pib_Per_Capita' ,'IDEB_anos_iniciais', 'IDEB_anos_finais']
...
while (fim_count <= 5570):
	try:
		...
		try:
			populacao_residente_df.res = populacao_residente_df.res.apply(lambda x: [str(item['res']['2017']) for item in x])
		except Exception as e:
			try:
				populacao_residente_df.res = populacao_residente_df.res.apply(lambda x: [str(item['res']['2017'])  if '2017' in item['res'] else  '-' for item in x])
			except Exception as e:
		...
		final_2017 = pd.concat([final_2017, populacao_residente_df])
		#time.sleep(2)
	except Exception as e:
		print(str(e))
		#print(populacao_residente_df.res[0])
		final_2017.to_excel("final_bkp.xlsx")
	#print(final_2009)

final_2017.columns = cols
final_2017.set_index('id_MUNICIPIO', inplace=True)
final_2017.to_excel("final_2017.xlsx")
